...
/**
 * Branded full-screen loader — matches the renter/admin apps' splash
 * treatment, built on the real brand artwork (see
 * src/frontend/components/Logo.tsx / public/emblems/ziyam-{plate,art}.svg)
 * rather than the old hand-drawn approximation. The ring is a genuine
 * stroke path, so it still stroke-draws in via CSS dasharray exactly like
 * before; the plate and the real cream Z-swoosh are filled shapes, so they
 * fade/scale in and out instead of tracing.
 */
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-[#2B1E17]">
      <div className="flex flex-col items-center gap-5">
        <svg viewBox="0 0 1024 1024" className="w-20 h-20 sm:w-24 sm:h-24" xmlns="http://www.w3.org/2000/svg">
          <defs>
          ...
          <circle cx="512" cy="512" r="470" fill="none" stroke="#F3E9D8" strokeOpacity="0.15" strokeWidth="11" />
          <circle cx="512" cy="512" r="481" fill={`url(#${plate})`} />
          <circle
            cx="512" cy="512" r="470" fill="none" stroke="#F3E9D8" strokeWidth="11" strokeLinecap="round"
            pathLength={100} strokeDasharray={100} className="animate-[ziyam-ring_1.6s_ease-in-out_infinite]"
          />
          {/* Static recenter+scale (see frontend Logo.tsx for why), on its
              own <g> outside the animated one so the CSS animation's
              transform (scale via keyframes) doesn't clobber it. */}
          <g transform="translate(512,512) scale(1.5) translate(-512,-358)">
          <g
            fill={`url(#${cream})`} fillRule="evenodd" style={{ transformOrigin: '50% 50%' }}
            className="animate-[ziyam-mark_1.6s_ease-in-out_infinite]"
          >
