...
/**
 * Standalone, reusable loading animation built on the real brand artwork
 * (same source as Logo.tsx / public/emblems/ziyam-{plate,art}.svg), not the
 * old hand-drawn approximation. The ring is a genuine stroke path, so it
 * still stroke-draws in an infinite loop via real SVG path animation
 * (motion's pathLength) exactly like before; the plate and the real cream
 * Z-swoosh are filled shapes, so they fade/scale in and out instead — a
 * plain stroke-trace doesn't apply to filled artwork. Used by
 * LoadingScreen.tsx for the full-screen splash, and available standalone
 * for inline loading states (buttons, cards, etc.) — just drop it in at
...
export default function ZiyamLogoLoader({ className, strokeColor = '#F3E9D8' }: { className?: string; strokeColor?: string }) {
  ...
  return (
    <svg viewBox="0 0 1024 1024" className={className} xmlns="http://www.w3.org/2000/svg">
      ...
      <circle cx="512" cy="512" r="470" stroke={strokeColor} strokeOpacity="0.15" strokeWidth="11" fill="none" />
      <motion.circle
        cx="512" cy="512" r="470" stroke={strokeColor} strokeWidth="11" strokeLinecap="round" fill="none"
        initial="hidden" animate="visible" variants={ringTrace}
      />
      {/* Static recenter+scale (see Logo.tsx for why) on its own <g>, separate
          from the motion-animated one below — motion drives that element's
          transform directly, and would otherwise clobber a transform
          attribute set statically on the same node. */}
      <g transform="translate(512,512) scale(1.5) translate(-512,-358)">
      <motion.g
        fill={`url(#${cream})`} fillRule="evenodd" style={{ transformOrigin: '50% 50%' }}
        initial="hidden" animate="visible" variants={markFade}
      >
