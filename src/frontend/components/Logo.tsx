...
export function LogoBadge({ className }: { className?: string }) {
  ...
  return (
    <svg viewBox="0 0 1024 1024" className={className} xmlns="http://www.w3.org/2000/svg">
      ...
      {/* The swoosh's own bounding box is centered at (512,358), not (512,512)
          — in the full artwork that's deliberate, leaving room for the
          wordmark underneath. Dropping the wordmark here left it looking
          small and off-center, floating in the upper half of the plate, so
          it's recentered on the plate's actual center and scaled up 1.5x to
          read with real presence at icon size. */}
      <g
        fill={`url(#${cream})`}
        fillRule="evenodd"
        filter={`url(#${emboss})`}
        transform="translate(512,512) scale(1.5) translate(-512,-358)"
      >
